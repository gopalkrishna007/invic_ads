...
class Uploaddata{
	...
	public function uploadAlltypesofImages($files,$folders)
	{
		$filename = "";
		$folder_structure = "/";
		if(count($folders)>0){
			...
			foreach ($folders as $folder) {
            	...
            	$_FILES['image']['size']= $files['size'];
				$config['upload_path'] = $folderpath;
				$config['allowed_types'] = 'jpg|png|jpeg|pdf|doc|docx|xls|xlsx|txt';
	...
	public function upload_multiple_images($files,$folders,$type=null)
	{
		...
		if(count($folders)>0)
		{
			...
			$folderpath = base_path . '/uploads/'.$folder_structure;
			$config['upload_path'] = $folderpath; 
			$config['allowed_types'] = 'gif|jpg|png|jpeg|xlsx|pdf|doc|docx|xls|xlsx';
	        /* $config['max_size']	= '5120'; */
	        $config['encrypt_name']	= TRUE;
