...
<div class="page-content">
   <div class="content">
      ...
      <div class="page-title">
         ...
         <h3>View <span class="semi-bold">Locations</span></h3>
      </div>
      <div class="row">
         <div class="col-md-12">
            <div class="grid simple ">
               <div class="grid-body no-border">
                  <table class="table no-more-tables">
                     ...
                     <tbody>
                        <?php if(!empty($locationData)){ $sno=1; foreach($locationData as $dat){ ?>					
                        <tr id="remove<?= $dat['id'] ?>">
                           <td class="v-align-middle"><?= $sno ?></td>
                           <td><span class="muted"><?= ucwords($dat['city_name']) ?></span></td>
						   <td><span class="muted"><?= ucwords($dat['location_name']) ?></span></td>
                           <td class="v-align-center">	
						   <span class="label <?= ($dat['location_status'] == 1)?'label-important':(($dat['location_status'] == 1)?'label-success':'') ?>"><?= ($dat['location_status'] == 1)?'Active':(($dat['location_status'] == 2)?'In Active':'') ?></span>
						   </td>
                           <td class="v-align-middle">
                              <div class="btn-group">
                                 ...
                                 <ul class="dropdown-menu">
                                    <li data-filter="camping" data-dimension="recreation"><a href="<?= base_url("admin/addlocation/".$dat['id']) ?>">Edit</a></li>
                                    ...
                                    <li data-filter="fishing" data-dimension="recreation" style="cursor: pointer;"><a class="sweet-2" data="<?= $dat['id'] ?>" data-status="<?= ($dat['location_status'] == 1)?'2':'1' ?>" onclick="_gaq.push(['_trackEvent', 'example', 'try', 'sweet-2']);"><?= ($dat['location_status'] == 1)?'Deactivate':'Activate' ?></a></li>
                                 </ul>
                              </div>
                           </td>
                        </tr>
        ...
        button.onclick = function() {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        type: "POST",
                        url: '<?= base_url("admin/deletelocation") ?>',
                        data: {
                            "id": id
                        },
                        dataType: "json",
        ...
        button.onclick = function() {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        type: "POST",
                        url: '<?= base_url("admin/updatelocation") ?>',
                        data: {
                            "id": id,
                            "status": status
                        },
