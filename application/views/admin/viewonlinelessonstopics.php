...
<li><a href="#" class="active">Online Lesson topics</a>
</li>
</ul>
<div class="page-title"> <i class="icon-custom-left"></i>
<h3>View/Edit <span class="semi-bold"> Online Lesson topics</span></h3>
</div>
<div class="row">
<div class="col-md-12">
...
<div class="grid-body no-border">
		...
		<table class="table no-more-tables">
			<thead>
				...
				<th style="width:10%">Topic Title</th>
			...
			<?php if(!empty($lessondata)){ $sno=1; foreach($lessondata as $data){ ?> 
				<tr>
					...
					<td><span class="muted"><?= ucwords($data['topic_title']) ?></span></td>
					<td class="v-align-middle"><?php if($data['topic_image'] != ''){ ?> <img src="<?= base_url("uploads/topic_images/".$data['topic_image']) ?>" style="width:30%" /> <?php } ?></td>
					<td class="v-align-middle">
					...
					<li data-filter="camping" data-dimension="recreation"><a href="<?= base_url("admin/uploadonlinelessontopics/".$data['id']) ?>">Edit</a></li>
        ...
        button.onclick = function() {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        type: "POST",
                        url: '<?= base_url("admin/deleteonlinetopic") ?>',
                        data: {
                            "id": id
                        },
                        dataType: "json",
        ...
        button.onclick = function() {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        type: "POST",
                        url: '<?= base_url("admin/updateonlinetopicstatus") ?>',
                        data: {
                            "id": id,
                            "status": status
                        },
